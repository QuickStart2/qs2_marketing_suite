...
patch(PivotModel.prototype, {
    ...
    _getFieldWithAggregator(fieldName) {
        const measure = this.metaData.measures[fieldName];

        if (measure && measure.aggregator) {
            return `${fieldName}:${measure.aggregator}`;
        }
    ...
    addComputedMeasure(id, field1, field2, operation, name, format) {
        ...
        const field1WithAgg = this._getFieldWithAggregator(field1);
        const field2WithAgg = this._getFieldWithAggregator(field2);

        const oper = operation.replace(/m1/g, field1WithAgg).replace(/m2/g, field2WithAgg);
    ...
    _fillComputedMeasuresData(subGroupData, config) {
        for (const cm of this._computed_measures) {
            ...
            const field = this.metaData.fields[cm.id]
            const field_str = cm.id + ":" + field.aggregator
            if (subGroupData.__count === 0) {
                subGroupData[field_str] = false;
            } else {
                subGroupData[field_str] = evalOperation(cm.operation, subGroupData);
            }
        }
    },
    ...
    async _getGroupsSubdivision(params, groupInfo) {
        ...
        for (let i = 0; i < measureSpecs.length; i++)
            if (measureSpecs[i].startsWith("__computed_")) {
                computed_measures.push(measureSpecs[i]);
                measureSpecs.splice(i, 1);
                i--;
            }
        const res = await super._getGroupsSubdivision(...arguments);
        Object.assign(measureSpecs, computed_measures);
        return res;
    },
